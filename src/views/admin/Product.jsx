    ...
    const handleEditSubmit = async () => {
        try {
            const body = {
                ...
                deskripsi: deskripsi,
                harga: harga,
                harga_kulak: hargaKulak,
                kategori_id: category,
                stok: stok
            };

            const response = await axios.put(`http://localhost:8080/barang/updatebarang/${selectedProduct.id}`, body, {
                headers: {
        ...
        <div>
            <CRow>
                <CCol xs={12}>
                    <CCard>
                        ...
                        <CCardBody>
                            <CButton onClick={() => setVisible(true)} color="primary" >Tambah Produk</CButton>
                            <CButton onClick={handleAddCategory} color="primary" className="ms-2">Tambah Kategori</CButton>
                                    ...
                                    <CTableHead>
                                        <CTableRow>
                                            ...
                                            <CTableHeaderCell onClick={() => handleSort('harga')}>
                                                Harga {sortConfig.key === 'harga' && (sortConfig.direction === 'ascending' ? '↑' : '↓')}
                                            </CTableHeaderCell>
                                            <CTableHeaderCell scope="col" onClick={() => handleSort('harga_kulak')} >
                                            Harga Kulak{sortConfig.key === 'harga' && (sortConfig.direction === 'ascending' ? '↑' : '↓')}
                                        </CTableHeaderCell>
                                            <CTableHeaderCell onClick={() => handleSort('stok')}>
                                                Stok {sortConfig.key === 'stok' && (sortConfig.direction === 'ascending' ? '↑' : '↓')}
                                            </CTableHeaderCell>
                                        ...
                                        {paginatedData.map((item, index) => (
                                            <CTableRow key={item.id}>
                                                ...
                                                <CTableHeaderCell scope="row">
                                                    <CCardImage height={100} width={100} src={"http://localhost:8080/uploads/" + item.gambar}></CCardImage>
                                                </CTableHeaderCell>
            ...
            <CModal visible={visible} onClose={() => setVisible(false)}>
                ...
                <CModalFooter>
                    ...
                    <CButton
                        color="primary"
                        onClick={selectedProduct ? handleEditSubmit : handleSubmit}
                    >
                            ...
                            <ul className="list-group">
                                {categories.map((category, index) => (
                                    <li key={index} className="list-group-item d-flex justify-content-between align-items-center">
                                        ...
                                        <CButton onClick={() => handleDeleteCategory(category.id)} color="danger" size="sm" className="ms-2">
                                            Hapus
                                        </CButton>
                                    </li>
                                ))}
                    ...
                    <CForm className="mt-3">
                        <div className="mb-3">
                            ...
                            <CInputGroup>
                                ...
                                />
                                <CButton color="primary" onClick={handleAddNewCategory}>Tambah</CButton>
                            </CInputGroup>
                        </div>
                    </CForm>
                </CModalBody>
                <CModalFooter>
                    <CButton color="secondary" onClick={() => setVisibleCategory(false)}>Close</CButton>
                </CModalFooter>
            </CModal>
        </div>
    );
